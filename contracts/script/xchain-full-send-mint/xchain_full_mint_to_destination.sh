...
BALANCE_HEX=$(cast call $HYPERLANE_TOKEN_ADDRESS "balanceOf(address)" $ESPRESSO_TREASURY_ADDRESS --rpc-url=$DESTINATION_CHAIN_RPC_URL)
...
echo "Treasury $ESPRESSO_TREASURY_ADDRESS synthetic tokens balance on destination chain before send: $BALANCE_SYNTHETIC_BEFORE wei"
...
echo "Sending $XCHAIN_AMOUNT_WEI tokens in wei from the source chain to the tokens to the recipient $ESPRESSO_TREASURY_ADDRESS on destination chain, waiting 10 sec for relayer service confirmation..."
sleep 10
...
echo "Recipient $ESPRESSO_TREASURY_ADDRESS synthetic tokens balance on destination chain after send: $BALANCE_DECIMAL_AFTER wei"
