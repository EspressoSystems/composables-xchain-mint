 ...
 * [  68:  133] Validator signature (1/1 signer)
 * [  133:  134] Espresso TEE type
 */
library MessageIdMultisigIsmMetadata {
    ...
    uint8 private constant ESPRESSO_TEE_TYPE_OFFSET = 133;
    uint8 private constant TEE_TYPE_LENGTH = 1;
     ...
     */
    function originMerkleTreeHook(bytes calldata _metadata) internal pure returns (bytes32) {
        return bytes32(_metadata[ORIGIN_MERKLE_TREE_OFFSET:ORIGIN_MERKLE_TREE_OFFSET + 32]);
    }
    ...
    function index(bytes calldata _metadata) internal pure returns (uint32) {
        return uint32(bytes4(_metadata[MERKLE_INDEX_OFFSET:MERKLE_INDEX_OFFSET + 4]));
    }
    ...
    /**
     * @notice Returns the number of signatures in the metadata. The signatures count is always 1 for espresso verifier
     * @param _metadata ABI encoded MessageId Multisig ISM metadata.
     * @return The number of signatures in the metadata.
     */
    function signatureCount(bytes calldata _metadata) internal pure returns (uint256) {
        uint256 signatures = _metadata.length - SIGNATURES_OFFSET - TEE_TYPE_LENGTH;
        require(signatures % SIGNATURE_LENGTH == 0, "Invalid signatures length");
        return signatures / SIGNATURE_LENGTH;
    }

    /**
     * @notice Returns the espresso tee type as bytes1.
     * @param _metadata ABI encoded Multisig ISM metadata.
     * @return The espresso tee type as bytes1.
     */
    function espressoTeeType(bytes calldata _metadata) internal pure returns (bytes1) {
        return bytes1(_metadata[ESPRESSO_TEE_TYPE_OFFSET:ESPRESSO_TEE_TYPE_OFFSET + TEE_TYPE_LENGTH]);
    }
}
