...
contract EspHypNativeSpec is HypNative {
     ...
     *     NOTE: `msg.value` will be greater than `amoundOrId`, since it includes funds to cover cross-chain gas payment.  The post-dispatch IGP hook handles cross-chain gas payment errors, so there is no need to check here if the user has supplied sufficient cross-chain gas funds. 
    ...
    function initiateCrossChainNFTPurchase(
        ...
        require(
            _amountOrId == NFT_SALE_PRICE,
            "Specified funds do not equal NFT sale price"
        );
