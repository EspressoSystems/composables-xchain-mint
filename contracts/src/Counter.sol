...
contract Counter {
    ...
    function icaTest() public payable {
        ...
        CallLib.Call memory call =
            CallLib.Call({to: bytes32(bytes20(0xa0Ee7A142d267C1f36714E4a8F75612F20a79720)), data: "hello", value: 0});
        ...
        uint256 gasPayment = gasPaymaster.quoteGasPayment(rariDomain, 50_000);
        uint256 gasPayment2 = InterchainAccountRouter(routerAddress).quoteGasPayment(rariDomain);
        // console.log(">>>> gasPayment (wei):", gasPayment);
        emit Event(gasPayment);
        IInterchainAccountRouter(routerAddress).callRemote{value: gasPayment2}(rariDomain, calls);
    }
     ...
     */
    function registerIcaRouter(address _icaRouter, uint32 _destinationDomain, address _remoteRouter, address _remoteIsm)
        private
    {
        ...
        InterchainAccountRouter(_icaRouter).enrollRemoteRouterAndIsm(_destinationDomain, routerKey, ismKey);
    }
}
